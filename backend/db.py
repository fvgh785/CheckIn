...
def init_db():
    ...
    try:
        with conn.cursor() as cur:
            ...
            try:
                cur.execute(
                    'ALTER TABLE knowledge_bases ADD UNIQUE INDEX idx_title_category (title, category)'
                )
            except Exception:
                pass  # 索引已存在
            cur.execute('''
            ...
            try:
                ...
                pass
            # 初始化默认知识库数据（幂等：title+category 唯一）
            try:
                feature_intro = (
                ...
                cur.execute(
                    'INSERT IGNORE INTO knowledge_bases (id, title, content, category, enabled) VALUES (%s, %s, %s, %s, 1)',
                    (str(uuid.uuid4()), '小程序功能介绍', feature_intro, 'feature')
                )
            except Exception:
                pass
